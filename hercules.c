...
void debug_print_rbudp_pkt(const char *pkt, bool recv) {
	...
	printf("%s [%u -> %u] Header: IDX %u, Path %u, Seqno %u\n", prefix,
		   ntohs(*src_port), ntohs(*dst_port), h->chunk_idx, h->path, h->seqno);
...
static void send_control_pkt(struct hercules_server *server,
							 ...
							 struct hercules_control_packet *control_pkt,
							 struct hercules_path *path, u16 src_port) {
	char buf[HERCULES_MAX_PKTSIZE];
	void *rbudp_pkt = mempcpy(buf, path->header.header, path->headerlen);
...
static void rx_send_acks(struct hercules_server *server, struct receiver_state *rx_state)
{
	...
	send_control_pkt(server, rx_state->session, &control_pkt, &path, rx_state->src_port);
...
static void
tx_send_initial(struct hercules_server *server, const struct hercules_path *path, int tx_slot, u16 dst_port, char *filename, size_t filesize, u32 chunklen, unsigned long timestamp, u32 path_index, bool set_return_path, bool new_transfer)
{
	...
	fill_rbudp_pkt(rbudp_pkt, UINT_MAX, PCC_NO_PATH, 0, (char *)&pld,
				   sizeof(pld.type) + sizeof(pld.payload.initial) +
					   pld.payload.initial.name_len,
				   path->payloadlen);
		...
		if (path->enabled) {
			...
			if (handshake_at < now) {
				if (atomic_compare_exchange_strong(
					...
					tx_send_initial(server, path, tx_slot,
									tx_state->session->dst_port, "abcd",
									tx_state->filesize, tx_state->chunklen,
									get_nsecs(), p, false, false);
				}
			}
		}
	}
...
static inline void tx_handle_send_queue_unit(
	...
	struct send_queue_unit *unit, u32 thread_id, u16 dst_port) {
	for(int i = 0; i < server->num_ifaces; i++) {
		tx_handle_send_queue_unit_for_iface(tx_state, xsks[i], server->ifaces[i].ifid, frame_addrs[i], unit, thread_id, dst_port);
	}
}
	...
	for (int s = 0; s < HERCULES_CONCURRENT_SESSIONS; s++) {
		...
		if (session_tx && session_tx->state == SESSION_STATE_PENDING) {
			if (now >
				...
				tx_send_initial(server, &pathset->paths[0], s,
								session_tx->dst_port, tx_state->filename,
...
static void tx_send_p(void *arg) {
	...
	while (1) {
		...
		tx_handle_send_queue_unit(server, session_tx->tx_state, args->xsks,
								  frame_addrs, &unit, args->id, session_tx->dst_port);
...
static void new_tx_if_available(struct hercules_server *server) {
	...
	struct sender_state *tx_state = init_tx_state(
		session, filesize, chunklen, server->rate_limit, mem, paths, 1, n_paths,
					...
					case CONTROL_PACKET_TYPE_INITIAL:;
						...
						if (rx_slot != -1 &&
							...
							if (parsed_pkt->flags &
								...
								rx_handle_initial(server, rx_state, parsed_pkt, rx_slot,
